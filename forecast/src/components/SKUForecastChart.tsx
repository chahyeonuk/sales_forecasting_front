...
export function SKUForecastChart({ selectedSKU, dateRange }: { selectedSKU: string; dateRange: string }) {
  return (
        ...
        <ComposedChart data={skuForecastData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
          ...
          <Tooltip 
            formatter={(value, name) => {
              ...
              if (nameStr.includes('qty')) {
                return [value + '개', nameStr.includes('actual') ? '실제 수량' : '예측 수량']
              } else {
                return [new Intl.NumberFormat('ko-KR').format(value as number) + '원', nameStr.includes('actual') ? '실제 매출' : '예측 매출']
              }
            }}
